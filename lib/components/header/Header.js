...
const Header = (props) => {
    ...
    const renderLeftComponent = (hasButton) => {
        if (!hasButton) {
            return (<Icon size={iconSize || defaultTheme.headerIconSize} isPlaceholder={true}/>);
        }
        return onLeftButtonPress ? (<IconButton onPress={onLeftButtonPress} size={iconSize || defaultTheme.headerIconSize} source={iconSources[0]}/>) : (<Icon size={iconSize || defaultTheme.headerIconSize} source={iconSources[0]}/>);
    };
    const renderRightComponent = (hasButton) => {
        if (!hasButton) {
            return (<Icon size={iconSize || defaultTheme.headerIconSize} isPlaceholder={true}/>);
        }
        return onRightButtonPress ? (<IconButton onPress={onRightButtonPress} size={iconSize || defaultTheme.headerIconSize} source={iconSources[iconSources.length - 1]}/>) : (<Icon size={iconSize || defaultTheme.headerIconSize} source={iconSources[iconSources.length - 1]}/>);
    };
    const renderBody = (hasLeftButton, hasRightButton, hasTitle) => {
        const _middleComponent = hasTitle ? (<Flex paddingSize="NONE" position="MIDDLE" fill>
        <Text style={_style.title} size={defaultTheme.headerTextSize} bold>
          {title || ""}
        </Text>
      </Flex>) : (<Spacer />);
        if (props.gradientColors && props.gradientColors.length > 1) {
            return (<LinearGradient colors={props.gradientColors} style={_style.gradient}>
          <Flex fill marginSize="NONE" style={_style.root} paddingSize={paddingSize} direction={"row"}>
        ...
        return (<Flex marginSize="NONE" fill style={_style.root} paddingSize={paddingSize} direction={"row"}>
        {renderLeftComponent(hasLeftButton)}
        {_middleComponent}
        {renderRightComponent(hasRightButton)}
      </Flex>);
