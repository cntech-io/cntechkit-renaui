...
export type HeaderLayout = "title-only" | "title-buttons-together" | "title-left-button-together" | "title-right-button-together" | "right-button-only" | "left-button-only" | "buttons-only";
...
export type HeaderHeightSize = keyof Partial<Omit<typeof SizeEnum, "NONE" | "XL" | "XXL">>;
export type IconSize = keyof Partial<Omit<typeof SizeEnum, "NONE" | "XL" | "XXL">>;
export type TextSize = keyof Partial<Omit<typeof SizeEnum, "NONE" | "XL" | "XXL">>;
export type FixedRowHeight = keyof Partial<Omit<typeof SizeEnum, "NONE" | "XL" | "XXL">>;
type BorderRadiusSize = keyof Partial<Omit<typeof SizeEnum, "XXL">>;
export type Theme = {
    position: {
        [k in Position]: object;
    };
    padding: {
        [k in PaddingSize]: number;
    };
    margin: {
        [k in MarginSize]: number;
    };
    headerHeight: {
        [k in HeaderHeightSize]: number;
    };
    iconSize: {
